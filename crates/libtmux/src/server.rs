...
mod tests {
    ...
    /// Negative test for the defect finding #6 reported: an unrecognized
    /// marker was silently dropped, so a listing with only an owner whose row
    /// the decoder could not place came back empty rather than an error.
    #[test]
